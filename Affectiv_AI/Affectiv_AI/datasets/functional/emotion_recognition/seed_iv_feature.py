...
def transform_producer(file_path: str, feature: list, num_channel: int,
    ...
    write_pointer = 0
    subject_queue = [] # recoding multiple trials for one subject
    # loop for each trial of one session of one subject
    for trial_id in trial_ids:
        ...
        for cur_feature in feature:
            trial_samples.append(
                samples[cur_feature +
                        str(trial_id)])  # channel(62), timestep of one trial(n), bands(5)
        trial_samples = np.concatenate(
            trial_samples,
            axis=-1)[:num_channel]  # channel(62), timestep of one trial(n), features(5*k)
        trial_samples = trial_samples.transpose((1, 0, 2))
        # timestep of one trial(n), channel(62), features(5*k)

        if before_trial:
            trial_samples = before_trial(trial_samples)  # a transform for a trial
        ...
        for i, clip_sample in enumerate(trial_samples):
            ...
            if not transform is None:
                t_eeg = transform(eeg=clip_sample)['eeg']  # this transform is for a sample of a trial
        ...
        if len(trial_queue) and after_trial:
            trial_queue = after_trial(trial_queue) # this transform is for a trial
...
def seed_iv_feature_constructor(root_path: str = './eeg_feature_smooth',
    ...
    session_list = ['1', '2', '3']
    file_path_list = []
    # get subject data file list, include different session and different subject
