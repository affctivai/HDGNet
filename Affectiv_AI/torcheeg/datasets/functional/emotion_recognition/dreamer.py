...
def transform_producer(subject: int,
                       trial_len: int,
                       mat_data: any,
                       chunk_size: int,
                       overlap: int,
    ...
    for trial_id in range(trial_len):
        ...
        trial_samples = trial_samples[:, :num_channel].swapaxes(1, 0)  # channel(14), timestep(n*128)
...
def dreamer_constructor(
    ...
    trial_len = len(
        mat_data['DREAMER'][0, 0]['Data'][0, 0]['EEG'][0, 0]['stimuli'][0, 0])  # 18
