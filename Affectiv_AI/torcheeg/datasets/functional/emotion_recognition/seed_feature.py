...
def transform_producer(file_name: str, root_path: str, feature: list,
    ...
    for trial_id in trial_ids:
        ...
        trial_samples = np.concatenate(
            trial_samples,
            axis=-1)[:num_channel]  # channel(62), timestep(n), features(5)
        trial_samples = trial_samples.transpose((1, 0, 2)) # timestep(n), channel(62), features(5)

        if before_trial:
            trial_samples = before_trial(trial_samples) # this is also a function for LDS or moving averages or other methods which transfer based on a trial
        ...
        if len(trial_queue) and after_trial:
            trial_queue = after_trial(trial_queue) # after_trial is a function for LDS or moving averages or other methods which transfer based on a trial
