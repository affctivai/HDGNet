...
class BandTransform(EEGTransform):
    ...
    # Obtain multiple bands of data after the butter filter
...
class BandSignal(BandTransform):
    ...
    def apply(self, eeg: np.ndarray, **kwargs) -> np.ndarray:
        '''
        Function:
            the "def apply" is for deal with eeg with filter and extract feature
            If you want to other types of feature extracting, please rewrite the "def apply" in subclasses
        '''
        band_list = []
        for low, high in self.band_dict.values():
            c_list = []
            for c in eeg:
                b, a = butter(self.order, [low, high],
                              fs=self.sampling_rate,
                              btype="band")
                c_list.append(self.opt(lfilter(b, a, c))) # opt is the "def" to calculate feature, like DE, PSD, if there is no handle, return eeg
            c_list = np.array(c_list)
            band_list.append(c_list)
