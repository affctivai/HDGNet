...
class KFoldPerSubjectCrossTrial:
    ...
    Args:
        ...
        stratified (bool): Whether to use StratifiedKFold to preserve the percentage of samples for each class. (default: :obj:`False`)
    ...
    def split_info_constructor(self, info: pd.DataFrame) -> None:
        ...
        sessions = list(set(info['session_id']))

        for session in sessions:
            session_info = info[info['session_id'] == session]
            for subject in subjects:
                subject_info = session_info[session_info['subject_id'] == subject]
                trial_ids = list(set(subject_info['trial_id']))

                if self.stratified:
                    # get label from each trial
                    label_ids = session_info[['trial_id', 'emotion']].drop_duplicates(['trial_id'])['emotion']
                else:
                    label_ids = None

                for fold_id, (train_index_trial_ids,
                              test_index_trial_ids) in enumerate(
                    self.k_fold.split(trial_ids, label_ids)):
                    ...
                    if self.stratified:
                        train_label_ids = np.array(
                            label_ids)[train_index_trial_ids].tolist()
                        test_label_ids = np.array(
                            label_ids)[test_index_trial_ids].tolist()
                    ...
                    write_file_train = os.path.join(self.split_path, f'train_subject_{subject}_fold_{fold_id}.csv')
                    write_file_test = os.path.join(self.split_path, f'test_subject_{subject}_fold_{fold_id}.csv')

                    if not os.path.exists(write_file_train):
                        train_info.to_csv(write_file_train, index=False)
                    else:
                        train_info.to_csv(write_file_train, mode='a', index=False, header=False)

                    if not os.path.exists(write_file_test):
                        test_info.to_csv(write_file_test, index=False)
                    else:
                        test_info.to_csv(write_file_test, mode='a', index=False, header=False)
